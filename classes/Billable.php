...
trait Billable
{
    ...
    public function charge($amount, array $options = [])
    {
        ...
        if (! array_key_exists('source', $options) && $this->offline_cashier_stripe_id) {
            $options['customer'] = $this->offline_cashier_stripe_id;
        }
    ...
    {
        return (bool) $this->offline_cashier_card_brand;
    }
    ...
    {
        return $this->offline_cashier_trial_ends_at && Carbon::now()->lt($this->offline_cashier_trial_ends_at);
    }
    ...
    public function invoice()
    {
        if ($this->offline_cashier_stripe_id) {
            try {
                return StripeInvoice::create(['customer' => $this->offline_cashier_stripe_id], $this->getStripeKey())->pay();
    ...
    public function upcomingInvoice()
    {
        try {
            $stripeInvoice = StripeInvoice::upcoming(
                ['customer' => $this->offline_cashier_stripe_id], ['api_key' => $this->getStripeKey()]
            );
    ...
    public function updateCardFromStripe()
    {
        ...
        if ($defaultCard) {
            ...
            $this->forceFill([
                'offline_cashier_card_brand' => null,
                'offline_cashier_card_last_four' => null,
            ])->save();
        }
    ...
    protected function fillCardDetails($card)
    {
        if ($card instanceof StripeCard) {
            $this->offline_cashier_card_brand = $card->brand;
            $this->offline_cashier_card_last_four = $card->last4;
        } elseif ($card instanceof StripeBankAccount) {
            $this->offline_cashier_card_brand = 'Bank Account';
            $this->offline_cashier_card_last_four = $card->last4;
        }
    ...
    {
        return ! is_null($this->offline_cashier_stripe_id);
    }
    ...
    public function createAsStripeCustomer($token, array $options = [])
    {
        ...
        $this->offline_cashier_stripe_id = $customer->id;
    ...
    {
        return StripeCustomer::retrieve($this->offline_cashier_stripe_id, $this->getStripeKey());
    }
    ...
    {
        return $this->honcode_cashier_stripe_id;
    }
